...
class Generation:
    ...
    messageGroups = [[],[],[],[],[], [8, 8], [12, 12], [15, 15], [19, 18], [20, 20], [25, 23]]
    ...
    def output_to_file(self, count = 0):
        if count > 0:
            self.fileName = "morse" + str(self.wordsPerMinute) + "wpm_everything" + str(count) + "0000"
            file = open(self.fileName + ".txt", "w+")
            file.write(self.message)
            file.close()
        else:
            self.fileName = "morse" + str(self.wordsPerMinute) + "wpm_everything0000"
            file = open(self.fileName + ".txt", "w+")
            file.write(self.message)
            file.close()
    ...
    def generate_morse_audio(self, count = 0):
        if count > 0:
            ...
            args = 'ebook2cw.bat ' + wpm + " morse" + wpm + 'wpm_everything' + str(count) + ' "morse' + wpm + 'wpm_everything' + str(count) + '0000.txt"'
            subprocess.call(args)
        else:
            wpm = str(self.wordsPerMinute)
            args = 'ebook2cw.bat ' + wpm + " morse" + wpm + 'wpm_everything "morse' + wpm + 'wpm_everything0000.txt"'
            subprocess.call(args)
