...
class Generation:
    ...
    messageGroups = [[],[],[],[],[], [9, 9], [12, 11], [16, 16], [20, 20], [24, 24], [27, 27], [33, 32], [36, 36], [40, 39], [44, 44], [47, 47]]
    ...
    usableCharacters = "ABCDEFGHIJKLMNOPQRSTUVWXYZäéöü"
    ...
    def generate_output(self):
        ...
        for i in range(self.messageGroups[self.wordsPerMinute][0]):
            sample = random.sample(self.usableCharacters, 5)
    ...
    def output_to_file(self, count = 0):
        if count > 0:
            self.fileName = "morse" + str(self.wordsPerMinute) + "wpm_characters" + str(count) + "0000"
            file = open(self.fileName + ".txt", "w+")
            file.write(self.message)
            file.close()
        else:
            self.fileName = "morse" + str(self.wordsPerMinute) + "wpm_characters0000"
            file = open(self.fileName + ".txt", "w+")
            file.write(self.message)
            file.close()
    ...
    def generate_morse_audio(self, count = 0):
        if count > 0:
            ...
            args = 'ebook2cw.bat ' + wpm + " morse" + wpm + 'wpm_characters' + str(count) + ' "morse' + wpm + 'wpm_characters' + str(count) + '0000.txt"'
            subprocess.call(args)
        else:
            wpm = str(self.wordsPerMinute)
            args = 'ebook2cw.bat ' + wpm + " morse" + wpm + 'wpm_characters "morse' + wpm + 'wpm_characters0000.txt"'
            subprocess.call(args)
