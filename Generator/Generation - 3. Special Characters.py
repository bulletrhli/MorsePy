...
class Generation:
    ...
    messageGroups = [[],[],[],[],[], [7, 6], [10, 9], [14, 13], [17, 16], [20, 19], [23, 22], [25, 25], [29, 29], [33, 32], [36, 35], [39, 38]]
    ...
    def generate_output(self):
        ...
        for i in range(self.messageGroups[self.wordsPerMinute][0]):
            sample = random.sample(self.special, 4)
            sample.append(random.choice(self.special))
            wordset = "".join(sample)
            wordblock += wordset + " "
            if breakCounter == 9:
                wordblock += "\n"
    ...
    def output_to_file(self, count = 0):
        if count > 0:
            self.fileName = "morse" + str(self.wordsPerMinute) + "wpm_special" + str(count) + "0000"
            file = open(self.fileName + ".txt", "w+")
            file.write(self.message)
            file.close()
        else:
            self.fileName = "morse" + str(self.wordsPerMinute) + "wpm_special0000"
            file = open(self.fileName + ".txt", "w+")
            file.write(self.message)
            file.close()
    ...
    def generate_morse_audio(self, count = 0):
        if count > 0:
            ...
            args = 'ebook2cw.bat ' + wpm + " morse" + wpm + 'wpm_special' + str(count) + ' "morse' + wpm + 'wpm_special' + str(count) + '0000.txt"'
            subprocess.call(args)
        else:
            wpm = str(self.wordsPerMinute)
            args = 'ebook2cw.bat ' + wpm + " morse" + wpm + 'wpm_special "morse' + wpm + 'wpm_special0000.txt"'
            subprocess.call(args)
