...
class Generation:
    ...
    messageGroups = [[],[],[],[],[], [8, 8], [14, 13], [17, 16], [21, 20], [26, 25], [29, 28], [34, 34], [37, 37], [42, 42], [46, 45], [49, 49]]
    ...
    def generate_output(self):
        ...
        for i in range(self.messageGroups[self.wordsPerMinute][0]):
            sample = random.sample(self.letters, 5)
            wordset = "".join(sample)
            wordblock += wordset + " "
            if breakCounter == 9:
                wordblock += "\n"
    ...
    def output_to_file(self, count = 0):
        if count > 0:
            self.fileName = "morse" + str(self.wordsPerMinute) + "wpm_letters" + str(count) + "0000"
            file = open(self.fileName + ".txt", "w+")
            file.write(self.message)
            file.close()
        else:
            self.fileName = "morse" + str(self.wordsPerMinute) + "wpm_letters0000"
            file = open(self.fileName + ".txt", "w+")
            file.write(self.message)
            file.close()
    ...
    def generate_morse_audio(self, count = 0):
        if count > 0:
            ...
            args = 'ebook2cw.bat ' + wpm + " morse" + wpm + 'wpm_letters' + str(count) + ' "morse' + wpm + 'wpm_letters' + str(count) + '0000.txt"'
            subprocess.call(args)
        else:
            wpm = str(self.wordsPerMinute)
            args = 'ebook2cw.bat ' + wpm + " morse" + wpm + 'wpm_letters "morse' + wpm + 'wpm_letters0000.txt"'
            subprocess.call(args)
